...
$dir2 = path_decode(@$_GET['dir'], $cfg->rootdir);
if(!is_dir($dir2)){
	...
	if($handle = opendir($dir2))
	{
		...
		{ 
			...
			if($filetype=="file")
			{
				...
				if(stripos($obj['type'], 'image') !== false && $obj['size'] <= $cfg->thumbnail_max_size)
				{
					try
					{
						$is = getimagesize($fn);
...
array_multisort($_order, SORT_ASC, $_order2, SORT_ASC, SORT_STRING, $arrfile);
...
if(count($arrdir)>0 || count($arrfile)>0)
{
	if(@$_GET['thumbnail'])
	{
        ...
        <div class="thumbimage" <?php if(@$cfg->thumbnail && @$val['size'] <= $cfg->thumbnail_max_size){ ?>style="background-image:url('tool-thumb-image.php?filepath=<?php echo rawurlencode($val['path']);?>')" <?php } ?>><div class="thumbimage-inner"><a href="javascript:;" title="<?php echo $val['name'];?>" onClick="return openDir('<?php echo str_replace("'", "\'", $val['path']);?>')"><img id="imageid-<?php echo $i;?>" src="style/images/trans96.gif" /></a></div></div>
        <div class="thumbcheck"><input type="checkbox" class="fileid" data-isdir="true" name="fileid[]" id="fileid-<?php echo $i;?>" value="<?php echo $val['path'];?>" /></div>
        ...
        <div class="thumbimage" <?php if($val['image_width'] && $val['image_height'] && stripos($val['type'], 'image')===0 && $cfg->thumbnail){ ?> style="background-image:url('tool-thumb-image.php?filepath=<?php echo rawurlencode($val['path']);?>')"<?php } ?>><div class="thumbimage-inner"><a href="javascript:;" title="<?php echo $val['name'];?>" onClick="return selectFile('<?php echo $val['url'];?>')"><img id="imageid-<?php echo $i;?>" src="style/images/trans96.gif" /></a></div></div>
        <div class="thumbcheck"><input type="checkbox" class="fileid" data-isdir="false" data-iszip="<?php echo ($val['type']=='application/zip')?'true':'false';?>" name="fileid[]" id="fileid-<?php echo $i;?>" value="<?php echo $val['path'];?>" /></div>
...
<thead>
<tr>
  <td width="8"><input type="checkbox" name="control-fileid" id="control-fileid" class="checkbox-selector" value="1"></td>
  <td width="8">Icon</td>
  <td>File Name</td>
  <td width="60" align="right">Size</td>
  <td width="70">MIME Type</td>
  <td width="40">Perms</td>
...
<tr class="row-data-dir row-<?php echo ($i%2)?'odd':'even';?>" data-file-name="<?php echo $val['name'];?>" data-file-location="<?php echo $val['location'];?>" data-file-type="dir">
  <td><input type="checkbox" class="fileid" data-isdir="true" name="fileid[]" id="fileid-<?php echo $i;?>" value="<?php echo $val['path'];?>" /></td>
...
<tr class="row-data-file row-<?php echo ($i%2)?'odd':'even';?>" data-file-url="<?php echo $val['url'];?>" data-file-name="<?php echo $val['name'];?>" data-file-location="<?php echo $val['location'];?>" data-file-type="<?php echo $val['type'];?>" data-file-size="<?php echo $val['size'];?>" data-image-width="<?php echo $val['image_width'];?>" data-image-height="<?php echo $val['image_height'];?>">
  <td><input type="checkbox" class="fileid" data-isdir="false" data-iszip="<?php echo ($val['type']=='application/zip')?'true':'false';?>" name="fileid[]" id="fileid-<?php echo $i;?>" value="<?php echo $val['path'];?>" /></td>
