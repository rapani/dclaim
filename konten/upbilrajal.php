   ...
   foreach ($_FILES['files']['name'] as $f => $name) {
    ...
    //$tahun = "20".$tahun;
       ...
       if ($_FILES['files']['error'][$f] == 0) {
        ...
        if((file_exists($dir))) {
        $up = move_uploaded_file($_FILES["files"]["tmp_name"][$f], "$dir/$name-BILLING.pdf");
        } else {
        mkdir($dir, null, true);
        $up = move_uploaded_file($_FILES["files"]["tmp_name"][$f], "$dir/$name-BILLING.pdf");
        }
        $input = mysqli_query($con, "UPDATE rjtl SET pdf_billing=1 WHERE no_sep='$name'");
  }
}
 if ($up){ $pesan = '<div class="alert alert-primary" role="alert">
  <h3> Terima Kasih :) Document Billing Berhasil di Upload </h3>
</div>';}
}
...
<div class="card">
  <h3 class="card-header">UPLOAD DOCUMENT BILLING RAWAT JALAN</h3>
  <div class="card-body">

<?php if (isset($pesan)) echo $pesan; ?>
 <form method="POST" enctype="multipart/form-data" autocomplete="off">
   <div class="form-group row">
     <label for="inputPassword" class="col-sm-2 col-form-label"><b> PDF BILLING </b> </label>
     <div class="col-sm-6">
